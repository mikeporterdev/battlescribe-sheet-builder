...
export function isBSForce(
  bsForces: { force: BSForce[] } | string,
): bsForces is { force: BSForce[] } {
...
export function isBSRule(
  bsRules: { rule: BSRule[] } | string,
): bsRules is { rule: BSRule[] } {
...
export function isWeaponProfile(
...
export function isPsykerProfile(
  profile: Profile<TypeName>,
): profile is PsykerProfile {
  return profile.typeName === "Psyker";
}

export function isUnitProfile(
  profile: Profile<TypeName>,
): profile is UnitProfile {
  return profile.typeName === "Unit";
}

export function isBSUnitProfile(
  bsProfile: BSProfile<BSCharacteristic>,
): bsProfile is BSProfile<BSUnitCharacteristic> {
